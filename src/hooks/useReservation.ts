import {
  customerReservationsAtom,
  reservationFormAtom,
  reservationLoadingAtom,
  reservationStepAtom,
  type ReservationFormData,
} from "@/atoms/customer-reservation";
import {
  ReservationService,
  type CreateReservationRequest,
} from "@/services/customer/reservation";
...
export const useReservation = () => {
  ...
  const createReservationMutation = useMutation({
    ...
    onSuccess: (response) => {
      showSuccess(
        `Reservation created successfully! Your reservation code is: ${response.reservation.reservationCode}`
      );
  ...
  const customerReservationsQuery = useQuery({
    ...
    queryFn: () =>
      ReservationService.getReservationsByPhone(
        auth?.user?.phoneNumber || formData.phoneNumber
      ),
    ...
    try {
      ...
      const reservationData: CreateReservationRequest = {
        ...
        reservationDate: new Date(
          formData.reservationDate + "T" + formData.timeSlot
        ).toISOString(),
  ...
  return {
    ...
    formData,
    currentStep,
    reservations: customerReservationsQuery.data?.reservations || {
      upcoming: [],
      past: [],
      total: 0,
    },
