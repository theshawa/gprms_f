...
export const Customer_CartPage: React.FC = () => {
  ...
  const handleOrderNow = async () => {
    ...
    try {
      const orderData = {
        customerId: Number(customer?.id) || 0, // 0 for guests
      ...
      showError("Failed to place your order. Please try again.");
    }
  };
  ...
  return (
    <>
      <div className="w-full mx-auto min-h-screen bg-white flex flex-col">
        ...
        {/* Cart Items */}
        <div
          className="flex-1 px-6 pt-6 pb-6 overflow-y-auto"
          style={{ paddingBottom: cartItems.length > 0 ? "140px" : "24px" }}
        >
          {cartItems.length === 0 ? (
      ...
      {showRemoveConfirmation && (
        <div
          data-layer="pop up"
          className="PopUp w-72 px-6 py-6 fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-3xl shadow-[0_25px_80px_rgba(0,0,0,0.4)] border border-gray-200 flex flex-col justify-start items-center gap-6 transform scale-105"
        >
          <div
            data-layer="title"
      ...
      {showClearAllConfirmation && (
        ...
        >
          ...
          >
            <div
              data-layer="Clear cart"
              className="ClearCart self-stretch text-center justify-start text-gray-900 text-xl font-semibold leading-normal"
            >
              Clear all items
            </div>
            <div
