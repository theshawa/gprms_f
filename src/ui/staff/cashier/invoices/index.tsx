import type {
  CashierInvoice,
  InvoiceOrder,
} from "@/interfaces/cashier-invoice";
...
export const Cashier_InvoicesPage: FC = () => {
  ...
  const [selectedInvoice, setSelectedInvoice] = useState<CashierInvoice | null>(
    null
  );
      ...
      <Box mb={3}>
        <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
          ...
          <TextField
            ...
            onChange={(e) =>
              updateSearchFilter("invoiceNumber", e.target.value)
            }
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
            ...
            <Box mb={2}>
              <Typography variant="body2" color="text.secondary">
                {Object.values(searchFilters).some((filter) => filter.trim())
                  ...
                  : `${filteredInvoices.length} ${activeTab} invoice${
                      filteredInvoices.length !== 1 ? "s" : ""
                    }`}
              </Typography>
            </Box>
